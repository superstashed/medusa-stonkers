...
client.once('ready', () => {
  ...
  if (discord.announcements.enabled) {
    ...
    listener.post('/', (req, res) => {
      if (req.headers.authorization == discord.announcements.pluginToken) {
        axios
          .get(`${medusa.baseUrl}/store/products/${req.headers.product}`)
          .then((res) => {
            if (discord.announcements.cheesyMessage) {
              res.status(200);
              return client.channels.cache.get(`${discord.announcements.channelId}`)
                .send({
                  content: `${discord.announcements.cheesyMessages[Math.floor(Math.random() * discord.announcements.cheesyMessages.length)]}${res.data.product.title}!\n${res.data.product.description}`,
                  files: [`${res.data.product.thumbnail}`]
                });
            }
            res.status(200);
            client.channels.cache.get(`${discord.announcements.channelId}`)
              .send({
                content: `New product published: ${res.data.product.title}\n${res.data.product.description}`,
                files: [`${res.data.product.thumbnail}`]
              });
          });
      }
